...
| (ctest) | `public_url`, `cloud_providers`, `range_integrity`, `database_persistence`, `categories`, `site_crawler`, `shell_integration`, `themes`, `download_task`, `download_core`, `license_token`, `offline_grace`, `account_signin`, `guard`, `guard_obfuscated` also run |
...
The released `.deb` (built by `.github/workflows/build.yml`) bundles Qt and libtorrent too, but takes glibc, libstdc++/libgcc and the GL stack from the host. Its `Depends` carries the real glibc/libstdc++ floors, derived at build time by `dpkg-shlibdeps` from the packaged binaries. An older release such as Ubuntu 22.04 is therefore refused by apt at install time instead of failing at launch; CI proves that in a clean 22.04 container.
