...
The backend boots with **no** Stripe or SMTP keys — billing runs in mock mode
(`constructEvent` is `JSON.parse`, so a webhook test is just a POST of the event
body) and emails are logged to the console. Production config is fail-closed:
`config/env.js` refuses to start on dev secrets, HTTP origins, an empty
`ADMIN_ALLOWED_IPS` or an unset `TRUST_PROXY`.
