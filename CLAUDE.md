...
| `nexa_pro_gate_test` | The course-site Pro gate through the real engine and IPC server: a CDN stream handed over from a course page (any `Referer`, whatever its case or position, a batch, a refresh capture, an IPC `download` or `links` message) is refused on Free with the website-trial wording and accepted on Pro, and a CDN file from any other page is never gated. Nothing is fetched — accepted downloads are held for the confirm prompt |
...
| `nexa_shell_integration_test` | The Windows Explorer hooks against a real registry, redirected under a throw-away test key so the machine's own shell is never touched: the open verb and the folder verb name THIS executable (quoted) and carry `--new-download --dir "%V"`, the handled file types appear under OpenWithProgids while their default handler is left alone, uninstalling removes our value from the shared OpenWithProgids key without taking another program's entry with it, a stale command path stops counting as registered (so the launch-time refresh repairs a moved install), and removing nothing is not a failure. A no-op body off Windows |
...
| (ctest) | `public_url`, `cloud_providers`, `range_integrity`, `database_persistence`, `categories`, `site_crawler`, `shell_integration`, `themes`, `download_task`, `download_core`, `license_token`, `offline_grace`, `account_signin`, `pro_gate`, `guard`, `guard_obfuscated` also run |
...
- **MEGA in-process:** `MegaGrabber` decrypts AES-128-CTR as bytes stream in (OpenSSL libcrypto, already a libtorrent dependency) and verifies MEGA's chunked CBC-MAC before marking done — no openssl CLI, no temp file, no GUI stall. `addDownload` routes MEGA **before** any yt-dlp branch, and `CloudProviders::isDirectFileUrl` is false for MEGA: yt-dlp cannot decrypt it, and it used to be handed MEGA links whenever it was installed.
...
- **The app signs in to an account; a licence key is the manual fallback.** `LicenseManager::beginSignIn()` runs OAuth's device-authorization flow against `/api/device/*`: the app shows an 8-character code, the person approves it on the website, and the app is handed a **device token** (`ndt_…`, in the OS credential store beside the key) that `/api/license/validate` takes in place of a key. The plan then follows the account, so a trial, an upgrade or a team invitation arrives with nothing to paste — and a Team member never holds the owner's key, which is the only copy of a secret that could leak. The two credentials are mutually exclusive on the wire and in the app: signing in clears a stored key, activating a key signs out. Settings → **Account** is the whole surface; the key row is folded behind "Use a license key instead". The one asymmetry worth remembering: `signed_out` makes the app forget its token, while `cancelled`/`expired` keep it signed in on Free so a renewal reaches it by itself — the account-shaped version of "nothing that merely runs out may answer expired". `LicenseManager` opens no browser itself (the UI does, from `signInCodeReady`), which is what keeps the licence layer testable with no GUI.
...
- **Entitlements come from the server:** `/api/license/validate` returns a `features` object (and embeds it in the signed token). `Entitlements` in `LicenseManager.h` defaults to the **Free** set, so a build that is offline before its first validation gates rather than leaks. `DownloadEngine::applyLicensePlan` reads it for the concurrency cap, AI rename and `authSiteDownloads`, on `entitlementChanged` (the plan name changed) **and** on `featuresChanged` (same plan, new limits) — the latter through a *queued* connection, because `featuresChanged` fires mid-validation before `setPlan`, and a synchronous `verifiedFeatures()` read in that gap looks like tampering to the guard, which then folds the install to Free for the session. Every 10th completed download (`LicenseManager::noteCompletedDownload`, counted in `license/completedDownloads`, so restarts and "clear completed" don't reset it) re-checks the plan with an **early heartbeat**, not `/validate`: `/validate` allows 10 calls an hour per IP, and a busy queue would exhaust it. The check is skipped with no seat held (a seatless beat is answered `seat_limit`) or within a minute of the last beat sent. Login-gated course sites (Udemy, Coursera, LinkedIn Learning) are refused in `addDownload` with `downloadBlocked`. Which sites those are is the `proOnly` flag in `resources/cloud_providers.json` (LinkedIn only under `proOnlyPaths` `/learning`), read by `CloudProviders::requiresPro` — deliberately **not** `isAuthSite`, which only means "downloads here use login cookies" and also covers Google, Vimeo and Apple Music; gating on it refused Free a public Drive file or Vimeo video. The page counts as much as the address: Coursera and Skillshare stream lectures from CDN hosts and the extension hands over the stream's own URL, so `DownloadEngine::proOnlySource` also checks every `Referer` the download carries, the gate runs ahead of the refresh capture, and IPC refuses a `links` message from a course page up front (one refusal, not one per link). Free gets the two core themes — `ThemeGalleryDialog` still renders every paid theme's real preview behind a PRO badge, because the gallery is the upgrade's best advert.
...
- **Explorer integration is per-user and never steals an association.**
  ...
  `setClassesRootForTesting` sends every key under a throw-away root, so
  `nexa_shell_integration_test` exercises all of this against a real registry
  without touching the machine's actual shell.
...
| Account sign-in | `src/license/LicenseManager.cpp` (`beginSignIn`/`signOut`), `CredentialStore.cpp` | `routes/device.js`, `models/DeviceAuth.js`, `utils/accountPlan.js` |
