...
export default function Reviews() {
  ...
  const columns = [
    { key: 'select', header: <label className="-my-3 flex h-11 w-11 cursor-pointer items-center justify-center"><input aria-label="Select all reviews" type="checkbox" className="h-4 w-4 accent-admin-accent" checked={allSelected} onChange={() => setSelected(allSelected ? new Set() : new Set(data.reviews.map((review) => review.id)))} /></label>, render: (review) => <label className="-my-3 flex h-11 w-11 cursor-pointer items-center justify-center"><input aria-label={`Select review ${review.id}`} type="checkbox" className="h-4 w-4 accent-admin-accent" checked={selected.has(review.id)} onChange={() => toggleSelected(review.id)} /></label> },
