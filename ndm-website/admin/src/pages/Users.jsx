...
function DetailsOutline() {
  return (
    <div className="space-y-6" role="status" aria-label="Loading account history">
      <div className="grid gap-3 sm:grid-cols-4">
        {[0, 1, 2, 3].map((i) => (
          <div key={i} className="rounded-xl border border-admin-border bg-admin-surface-2/60 p-3">
            <Skeleton className="h-3 w-12 rounded" />
            <Skeleton className="mt-2 h-4 w-24 rounded" />
          </div>
        ))}
      </div>
      {['w-28', 'w-32', 'w-20'].map((w) => (
        <div key={w}>
