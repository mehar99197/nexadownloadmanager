...
import { downloadCsv, formatDate, formatDateTime } from '../utils.js';

const LIMIT = 12;
const EMPTY_CREATE = { name: '', email: '', password: '', role: 'user', plan: 'free' };
...
export default function Users() {
  ...
  const [form, setForm] = useState({ banned: false, emailVerified: false, role: 'user', plan: 'free' });
  ...
  const columns = [
    ...
    { key: 'role', header: 'Role', render: (user) => <Badge tone={user.role === 'admin' ? 'info' : 'default'}>{user.role}</Badge> },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="admin-card grid gap-3 lg:grid-cols-[1.4fr_repeat(3,0.7fr)_auto] lg:items-end">
        ...
        <label className="block"><span className="admin-label">Role</span><select className="admin-input" value={filters.role} onChange={(event) => { setFilters((current) => ({ ...current, role: event.target.value })); setPage(1); }}><option value="">All roles</option><option value="user">User</option><option value="admin">Admin</option></select></label>
      ...
      <Modal open={createOpen} onClose={() => !saving && setCreateOpen(false)} title="Create user" footer={(<><Button variant="ghost" onClick={() => setCreateOpen(false)} disabled={saving}>Cancel</Button><Button onClick={createUser} disabled={saving || !createForm.name || !createForm.email || createForm.password.length < 8}>{saving ? 'Creating...' : 'Create user'}</Button></>)}><div className="space-y-4"><Input label="Name" name="newName" value={createForm.name} onChange={(event) => setCreateForm((current) => ({ ...current, name: event.target.value }))} /><Input label="Email" name="newEmail" type="email" value={createForm.email} onChange={(event) => setCreateForm((current) => ({ ...current, email: event.target.value }))} /><Input label="Temporary password" name="newPassword" type="password" hint="At least 8 characters" value={createForm.password} onChange={(event) => setCreateForm((current) => ({ ...current, password: event.target.value }))} /><div className="grid gap-4 sm:grid-cols-2"><label className="block"><span className="admin-label">Role</span><select className="admin-input" value={createForm.role} onChange={(event) => setCreateForm((current) => ({ ...current, role: event.target.value }))}><option value="user">User</option><option value="admin">Admin</option></select></label><label className="block"><span className="admin-label">Plan</span><select className="admin-input" value={createForm.plan} onChange={(event) => setCreateForm((current) => ({ ...current, plan: event.target.value }))}><option value="free">Free</option><option value="pro">Pro</option><option value="team">Team</option></select></label></div></div></Modal>
