...
import SharingQueue from './subscriptions/SharingQueue.jsx';
import { SkeletonText } from '../components/Skeleton.jsx';
import { downloadCsv, exportTruncationNotice, formatDate } from '../utils.js';
...
export default function Subscriptions() {
  ...
  const exportSubscriptions = async () => {
    setExportNotice('');
    try {
      const response = await api.get('/admin/subscriptions/export', { params: { q: query || undefined, status: status || undefined, plan: plan || undefined } });
      const rows = await unwrap(response);
      setExportNotice(exportTruncationNotice(response, (rows || []).length, 'subscriptions'));
  ...
  const columns = [
    ...
    // A sharing suspension leaves status 'active' (the app is told its seats are
    // full instead), so it is shown beside the status rather than hidden by it.
    { key: 'status', header: 'Status', render: (subscription) => <span className="flex flex-wrap items-center gap-1.5"><Badge status={subscription.status} />{subscription.sharing_suspended_at ? <Badge tone="danger">sharing suspended</Badge> : null}</span> },
  ...
  return (
    <div className="space-y-6">
      ...
      {exportNotice && <div role="status" className="rounded-xl border border-admin-warning/30 bg-admin-warning/10 px-4 py-3 text-sm text-admin-warning">{exportNotice}</div>}
      <SharingQueue onChanged={loadSubscriptions} />
