    ...
    <div className="flex min-h-screen bg-admin-bg text-admin-text">
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-72 flex-col border-r border-admin-border bg-admin-sidebar transition-transform md:static md:z-auto md:w-60 md:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex h-16 items-center gap-2 border-b border-admin-border px-5">
          <img src="/admin/nexa-logo-final.svg" alt="" className="h-9 w-9 rounded-xl" aria-hidden="true" />
          <div className="leading-tight">
            <p className="text-sm font-bold tracking-tight">Nexa<span className="bg-gradient-to-r from-accent-400 to-admin-cyan bg-clip-text text-transparent"> Admin</span></p>
            <p className="text-[11px] text-admin-faint">Control panel</p>
          </div>
        </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-16 items-center justify-between border-b border-admin-border bg-admin-surface/80 px-6 backdrop-blur-xl">
          <div className="flex items-center gap-3">
            ...
            <h1 className="mt-0.5 text-sm font-semibold text-admin-text">Administration</h1>
            </div>
          </div>
          <div className="flex items-center gap-4 text-xs text-admin-faint"><span className="hidden text-admin-muted sm:inline">{admin?.name || 'Administrator'}</span><span className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-admin-success shadow-[0_0_10px_rgba(69,223,193,0.9)]" />Live system</span></div>
        </header>
