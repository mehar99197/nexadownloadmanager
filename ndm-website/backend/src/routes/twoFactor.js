 ...
 * Resolves to { ok, usedRecovery, remaining } — the caller persists
 * `remaining` when a recovery code was consumed. Async because recovery codes
 * are bcrypt-compared; the TOTP path is tried first and costs nothing.
 */
...
async function retireLegacyRecoveryCodes(user) {
  ...
  await User.update(user.id, { totpRecovery: JSON.stringify(kept) });
...
function mountTwoFactor(router, { realm, secret, eligible, finishLogin, audit, gate }) {
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (result.usedRecovery) {
        // Whatever is left stays, legacy or not: a recovery sign-in says
        // nothing about whether the authenticator still exists, so taking the
        // other codes away here could lock the account out.
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      // The stored set is replaced outright — a used-up one, or the pre-bcrypt
      // SHA-256 one — so a recovery code offered as the proof is spent with it.
      const { codes, hashes } = await totp.generateRecoveryCodes();
...
module.exports = {
  mountTwoFactor, signChallenge, verifyChallenge, checkCode, twoFactorState, retireLegacyRecoveryCodes,
};
