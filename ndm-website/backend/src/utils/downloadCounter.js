 ...
 * and the one-byte "bytes=0-0" is the size probe the desktop app sends before
 * its real segmented transfer, which would otherwise count every app download
 * twice (rule 2 would fold it into the transfer that follows within the
 * window, but a probe is not a start whatever follows it).
 */
function isFreshStart(rangeHeader) {
  const range = String(rangeHeader || '').trim();
  if (range === '') return true;
  return /^bytes=0-/.test(range) && !/^bytes=0-0$/.test(range);
}
