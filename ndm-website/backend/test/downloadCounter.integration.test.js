...
test('download counter', async (t) => {
  ...
  await t.test('the app\'s one-byte "bytes=0-0" probe is a size probe too', async () => {
    // The desktop updater asks for byte 0 alone to learn the size before its
    // real segmented transfer; a range from byte 0 that stops at byte 0 is not
    // a start. Rule 2 would fold it into the transfer that follows, but only
    // inside the window — and an app that sizes the file and then waits for
    // Wi-Fi is one download, not two.
    const before = await countOf();
    await get(baseUrl, '/api/releases/download/windows', { ip: '203.0.113.71', range: 'bytes=0-0' });
    assert.equal(await countOf(), before, 'a one-byte probe must not count');
    // The real start from that address, afterwards, is the download.
    await get(baseUrl, '/api/releases/download/windows', { ip: '203.0.113.71', range: 'bytes=0-' });
    assert.equal(await countOf(), before + 1);
  });
