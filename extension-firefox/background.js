// Nexa integration — background script (Firefox MV3 event page; the Chromium
// copy is the MV3 service worker. Same code, different background container).
// Responsibilities:
...
async function injectIntoExistingTabs() {
  ...
  for (const tab of tabs) {
    ...
    try {
      // Firefox MV3 has `scripting`; the fallback keeps an older MV2 build working.
      if (X.scripting?.executeScript)
        await X.scripting.executeScript({ target: { tabId: tab.id }, files: ["content.js"] });
      else
        await X.tabs.executeScript(tab.id, { file: "content.js", runAt: "document_idle" });
    } catch (_) {
      // Browser-internal pages and restricted frames reject script injection.
    }
  }
}
